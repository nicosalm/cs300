...
public class Course implements Comparable<Course> {
  ...
  public Course(String deptName, int courseNum, int numCredits, int seats) {
    // check if any of the arguments are invalid and throw an exception if so
    ...
    if (numCredits < 1 || numCredits > 5) {
      throw new IllegalArgumentException("Number of credits must be between 1 and 5");
    }
    if (seats < 0) {
      throw new IllegalArgumentException("Number of seats must be non-negative");
    }

    // if we get here, all arguments are valid, so set the fields
  ...
  public void setProfessor(String profName, double rating) throws IllegalArgumentException {
    // if the professor is not null, check that the rating is valid
  ...
  public void setSeatsAvailable(int numSeatsAvailable) {
    // check that the number of seats is valid
    if (numSeatsAvailable < 0) {
      throw new IllegalArgumentException("Number of seats must be non-negative");
    }
